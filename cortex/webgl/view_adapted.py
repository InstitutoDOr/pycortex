...
def show_adapted(data, types=("inflated",), recache=False, cmap='RdBu_r', layout=None,
    ...
    html = FallbackLoader([serve.cwd]).load("mir.html")
    db.auxfile = data
    ...
    class PickerHandler(web.RequestHandler):
        def get(self):
            vid = int(self.get_argument("voxel"))
            vert = int(self.get_argument("vertex"))
    ...
    if open_browser:
        webbrowser.open("http://%s:%d/mir.html"%(serve.hostname, server.port))
