var dataset = (function(module) {
    ...
    module.DataView = function(json) {
        ...
        this.loaded = $.Deferred().done(function() { console.log("loaded deferred done"); $("#dataload").hide(); });
    ...
    module.DataView.prototype.init = function(uniforms, meshes, rois, frames) {
        ...
        deferred.done(function() {
            console.log("data loaded NO of datasets: "+this.data.length) 
    ...
    module.BrainData = function(json, images) {
        ...
        var loadmosaic = function(idx) {
        	console.log("loadmosaic called");
            var img = new Image();
            img.addEventListener("load", function() {
                var tex;
                if (this.raw) {
                    console.log("passed in this.raw");
                    tex = new THREE.Texture(img);
                    tex.premultiplyAlpha = true;
                } else {
                    console.log("this inside loadmosaic: "+JSON.stringify(this, null, 4));
		    console.log("Calling canvas imagedata (correspondent to extractValPixels): "+img.src);
                ...
                if (this.textures.length < this.frames) {
                	console.log("textures length less than frames");
                    this.loaded.notify(this.textures.length);
                    loadmosaic(this.textures.length);
                } else {
                	console.log("calling loaded resolve from eventlistener load(funcToPixels correspondent)");
                    this.loaded.resolve();
                    console.log("return resolve from eventlistener load(funcToPixels correspondent)");
                }
            }.bind(this));
            console.log("loadmosaic after eventlistener added");
